...
import { fasilitasEdit, fasilitasEditFormData } from '@/validations';
...
import { useGetFasilitasGetId } from '@/api';
...
const TambahBerita = () => {
    ...
    const {
        ...
        control,
        formState: { errors },
    } = useForm<fasilitasEditFormData>({
        resolver: zodResolver(fasilitasEdit),
    });

    // GET ONE SLUG
    // Integrasi API
    const { id } = useParams();
    const { data: dataUser } = useGetFasilitasGetId(id as string);
    ...
    const onSubmit: SubmitHandler<fasilitasEditFormData> = async (data) => {
        ...
        try {
            await axiosPrivate.put(`/facility/update/${id}`, formData, {
                headers: {
