...
import { useGetKategoriFilter, useGetPelatihanGetId } from '@/api';
...
const EditPelatihan = () => {
    ...
    // Integrasi API
    const { id } = useParams();
    const { data: dataUser } = useGetPelatihanGetId(id as string);
    ...
    const onSubmit: SubmitHandler<pelatihanFormData> = async (data) => {
        ...
        formData.append('linkModule', data.linkModule);
        ...
        try {
            ...
            // alert
            navigate.push('/layanan-ketenagakerjaan/pelatihan');
            // reset();
        } catch (error: any) {
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between gap-2 md:lg-3 lg:gap-5">
                        ...
                        <div className="flex flex-col mb-2 w-full md:w-1/2">
                            ...
                            <Input
                                ...
                                {...register('linkModule')}
                                className={`${errors.linkModule ? 'border-red-500' : ''}`}
                            />
                            {errors.linkModule && (
                                <HelperError>{errors.linkModule.message}</HelperError>
                            )}
                        </div>
                    </div>
                    {/*  */}
